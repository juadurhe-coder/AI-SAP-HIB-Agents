...
# DRY helper: aplica lista desplegable de Excel a un rango de celdas
function Add-DropdownValidation {
    param(
        [object]$Range,
        [string]$ValueList,
        [string]$ColumnName
    ...
    try {
        ...
        $validation.Add(3, 1, 1, $ValueList)
        $validation.IgnoreBlank = $true
        $validation.InCellDropdown = $true
    } catch {
        Write-Warning "No se pudo agregar desplegables a ${ColumnName}: $_"
    }
}
...
try {
    ...
    for ($col = 1; $col -le $colCount; $col++) {
        ...
        # --- Agregar Listas Desplegables (DRY via Add-DropdownValidation)
        if ($headerVal -eq "Status") {
            Add-DropdownValidation -Range $colRange -ValueList "Pending;In Progress;Completed" -ColumnName "Status"
        }

        if ($headerVal -eq "Priority") {
            Add-DropdownValidation -Range $colRange -ValueList "High;Medium;Low" -ColumnName "Priority"
        }
