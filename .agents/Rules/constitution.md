...
### 3.4 CONGELACIÓN Y CONSERVACIÓN DE DOCUMENTOS DE FASES PREVIAS (Document Freeze & Token Optimization)
- **Prohibición de Edición Retrospectiva:** Cuando un proyecto avanza a una fase posterior (ejemplo: de Propuesta `01_PR_*` a Diseño/Especificación `02_BPD_*` / `03_FS_*`), todos los documentos pertenecientes a las fases finalizadas u homologadas quedan automáticamente en estado **CONGELADO (LOCKED / Read-Only)**.
